...
\begin{question}[subtitle = You should be above to solve this]{1}
  \begin{enumerate}[leftmargin=*]
    \item
...
\begin{question}[subtitle = Multiple Choice zu reellen symmetrischen Bilinearformen]{2}
  ...
  \begin{enumerate}[leftmargin=*]
    ...
    \item
      Ist $V$ endlichdimensional, so gilt für eden Untervektorraum $U \subseteq V$ die Gleichung $\dim V = \dim U + \dim U^\perp$
    \item
...
\begin{question}[subtitle = Matrizen als Bilinearformen und lineare Abbildungen]{2}
  ...
  \begin{enumerate}[leftmargin=*]
    ...
    \item
      Zeigen Sie die Gleihheit $\Mat_\mc{B}(b) = \Mat_{\mc{B}, \mc{B}^*}(B)$.
...
\begin{question}[subtitle = Symbolmanipulation und kanonische Abbildungen]{4}
  ...
  \item
      ...
      ein Isomorphismus von $K$-Vektorräumen ist.
    \item
      Konstruieren Sie mithilfe der vorherigen Aufgabenteile einen Isomorphismus von $K$-Vektorräumen $T_{V,W} \colon \Hom(V, W^*) \to \Hom(W, V^*)$, so dass das folgende Diagram kommutiert:
      \[
