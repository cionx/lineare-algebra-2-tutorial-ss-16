...
%   \begin{enumerate}[leftmargin=*]
%     \item
%       Es sind zwar die Wegzusammenhangskomponenten, aber $\GL_n(\Rbb)_-$ ist keine Untergruppe.
%     \item
%       Falsch: $\Orthogonal(n)$ enthält zwar die Einheitsmatrix (da eine Untergruppe), ist aber keine Wegzusammenhangskomponente von $\GL_n(\Rbb)$.
%     \item
%       Ja, denn normierte Vektorräume sind immer wegzusammenhängend; für beliebige $A, B \in \orthogonal_n(\Rbb)$ ist $\gamma \colon [0,1] \to \orthogonal_n(\Rbb)$, $t \mapsto A + t(B-A)$ ein stetiger Weg von $A$ nach $B$.
%     \item
%       Nein: Dieser Schnitt ist $\SOrthogonal(n)$, also wegzusammenhängend.
%     \item
%       Nein, es ist keine Untergruppe, da etwa $I \notin G$.
%     \item
%       Falsch, siehe etwa $\{1,-1\}$ oder $\Orthogonal(n)$.
%     \item
%       Ja, dieser Schnitt ist $\SOrthogonal(n)$.
%     \item
%       Die Gruppe ist $\SOrthogonal(2)$, und somit wegzusammenhängend.
%       Alternativ lässt sich der Drehwinkel stetig ändern.
%   \end{enumerate}
% \end{solution}
